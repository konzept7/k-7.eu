...
export default function References() {
  ...
  const [references, setReferences] = useState<
    Array<{ id: number; attributes: ReferenceResponseItem }>
  >([]);
  ...
  return (
    <Stack spacing={10} my={30} ref={containerRef} px={10}>
      <Text ta="center" italic color="dimmed" size="sm">
        {t("ourClients")}
      </Text>
      <Center>
        <Group spacing={50}>
          {references.map((c, i) => {
            ...
            return (
              <Box
                ...
                className={cx(classes.reference, {
                  [classes.intersecting]: containerEntry?.isIntersecting,
                })}
              >
                <Center h="100%">
                  <Stack>
                    <Image
                      ...
                      alt={c.attributes.logo?.data.attributes?.alternativeText}
