...
const useStyles = createStyles((theme, { variant }: ContactIconStyles) => {
  ...
  return {
    wrapper: {
      display: "flex",
      backgroundColor:
        theme.colorScheme === "dark" ? theme.colors.brand[9] : theme.white,
    ...
    <CopyButton value={description} timeout={5000} {...others}>
      {({ copied, copy }) => (
        <Tooltip
          ...
          label={copied ? "Kopiert!" : "In die Zwischenablage"}
          withArrow
          position="right"
        >
          <div
            onClick={copy}
            style={{ cursor: copyDisabled ? undefined : "pointer" }}
...
export default function ContactSection() {
  ...
  const contactData = [
    { title: t("contact.email"), description: "info@k-7.eu", icon: IconAt },
    {
      ...
      description: "Roonstraße 23a, 76137 Karlsruhe",
      icon: IconMapPin,
    },
    {
      title: t("contact.hours"),
      description: "8.00 bis 17.00 Uhr",
      icon: IconSun,
      copyDisabled: true,
    },
  ];
  ...
  const form = useForm({
    ...
    validate: {
      name: (value) =>
        value && value.length > 5 ? null : t("contact.nameNecessary"),
      message: (value) =>
        value && value.length > 5 ? null : t("contact.betterWriteSomething"),
      _subject: (value) =>
        value && value.length > 5 ? null : t("contact.whatIsItAbout"),
      _replyto: (value) =>
        /^\S+@\S+$/.test(value) ? null : t("contact.invalidEmail"),
    },
  });
  ...
  return (
    ...
    >
      <Title align="center" color={theme.white} order={2} my="xl">
          ...
          <div className={classes.fields}>
            <SimpleGrid cols={2} breakpoints={[{ maxWidth: "sm", cols: 1 }]}>
              ...
              <TextInput
                ...
                name="_replyto"
                placeholder="info@k-7.eu"
                {...form.getInputProps("_replyto")}
                required
              />
            </SimpleGrid>
            ...
            />
            <Checkbox mt="md" name="gdpr" label={t("contact.gdpr")} required />
            <input
              type="hidden"
              name="_from"
              {...form.getInputProps("_from")}
            />
