...
export const getDepartmentByRoute = async (route: string, locale: string = "de") => {
  const queryUrl = `${import.meta.env.VITE_CMS}/api/departments?locale=${locale}&populate[0]=cover,skills,icon,projects[1]=skills.technologies,skills.icon,projects.author,projects.cover,projects.departments&populate[2]=projects.author.thumbnail&filters[route][$eqi]=${route}`
  const request = await fetch(queryUrl, {
    method: 'GET', headers
  })
  const response = await request.json() as DepartmentListResponse
